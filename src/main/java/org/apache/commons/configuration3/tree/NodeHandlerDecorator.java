 ...
 * </p>
 * <p>
 * This class implements all methods of the {@code NodeHandler} interface by delegating to another instance. This is
 * convenient if specific functionality of a {@code NodeHandler} is to be adapted for a special use case. Concrete sub
...
public abstract class NodeHandlerDecorator<T> implements NodeHandler<T> {
    ...
    public <C> List<T> getMatchingChildren(final T node, final NodeMatcher<C> matcher, final C criterion) {
        return getDecoratedNodeHandler().getMatchingChildren(node, matcher, criterion);
    }

    @Override
    public <C> int getMatchingChildrenCount(final T node, final NodeMatcher<C> matcher, final C criterion) {
        return getDecoratedNodeHandler().getMatchingChildrenCount(node, matcher, criterion);
    }
